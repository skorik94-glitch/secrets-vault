#!/usr/bin/env node
// hush setup — print (or write) the MCP config for a given client.
...
export const SERVER = { command: "npx", args: ["-y", "github:skorik94-glitch/hush", "mcp"] };
...
export function mergedConfig(existing, name = "hush", entry = SERVER) {
  const out = { ...(existing || {}) };
...
const jsonSnippet = () => JSON.stringify({ mcpServers: { "hush": SERVER } }, null, 2);
...
function instructions(client) {
  ...
  if (c.kind === "plugin") {
    ...
    L.push("  claude plugin marketplace add skorik94-glitch/hush");
    L.push("  claude plugin install hush@hush");
  } else if (c.kind === "codex") {
    L.push("OpenAI Codex:");
    L.push("  codex mcp add hush -- npx -y github:skorik94-glitch/hush mcp");
  } else if (c.kind === "json") {
    L.push(`${c.label}: add to ${c.path}`);
    L.push(jsonSnippet());
    L.push(`(or run: hush setup ${client} --write)`);
  } else {
...
function main() {
  ...
  if (!client || client === "help") {
    process.stdout.write("hush setup <client> [--write]\n\nClients: " + Object.keys(CLIENTS).join(", ") + "\n\n");
  ...
  if (write && c.kind === "json") {
    ...
    writeJsonClient(abs);
    process.stdout.write(`Wrote hush MCP server to ${abs}\nRestart ${c.label} to load it.\n`);
    return;
  }
